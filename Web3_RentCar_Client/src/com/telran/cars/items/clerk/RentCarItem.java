...
public class RentCarItem extends RentCompanyItem {
	...
	public void perform() {
		...
		LocalDate rentDate = inOut.inputDate("Enter initial date ", format);
		if (rentDate == null)
			return;
		// check rentDate>=current date!
		Integer rentDays = inOut.inputInteger("Enter rent days from " +MIN_RENT_DAYS+" till " + MAX_RENT_DAYS, MIN_RENT_DAYS, MAX_RENT_DAYS);
		if (rentDays == null)
			return;
